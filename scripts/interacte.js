...
const getAllInvestmentPools = async () => {
    ...
    const allInvestmentPools = await contract.connect(account1)._investementIds();
    for(let index = 1; index <= allInvestmentPools; index++){
      const investDetail = await contract.connect(account1).getInvstementPoolDetail(index);
